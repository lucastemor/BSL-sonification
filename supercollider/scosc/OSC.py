...
def OSCArgument(next):
    ...
    if type(next) == type(""):        
        ...
        # print(OSCstringLength,'of type',type(OSCstringLength))
        reply = ">%ds"%(OSCstringLength)
        # reply = bytes(reply,'ascii')
        binary  = struct.pack(reply, next.encode('utf-8')) #         # binary  = struct.pack(">%ds" % (OSCstringLength), next) # OG
        tag = "s"
