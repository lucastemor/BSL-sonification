...
def start( exedir='', port=57110, inputs=2, outputs=2, samplerate=44100, verbose=0,
    ...
    #start sc server in new terminal window -- works on linux only, will have to modift command for other os.
    #Allows suercollider to be run in the background so you do not need to boot gui every time 
...
"""
'''
Lucas added these functions Dec 2020 for the following conveniences:
    -avoid having to open supercollider gui just to run server
    -avoid having to open jackqt gui to set up patches for recording
    -not working right now .. boots okay but not recording :'-(
    -reason might be because we are unecssarily starting a new jack server ? but i haven't thought this through
    - should also add function to change default input and output device to pulse 
'''


def boot_jack():
    '''
    We are assuming that jack is already configured properly to record
    If not, go through the debug script to figure it out. This only works on linux
    Preset is sc3 for me, but may be differnet depending on what you have named it
    '''
    preset = 'sc3'
    os.system('qjackctl --start & --preset=[{}]'.format(preset))
    time.sleep(5)

def boot_sc_server():
    '''
    will open sc server in new gnome window to avoid having to launch supercollider gui
    '''
    os.system('gnome-terminal -- sclang  {}/init_server.scd'.format(os.path.dirname(__file__)))
    time.sleep(5)

def connect_supercollider_to_jack(record=True,monitor=True):
    commands = []
    if monitor:
        commands.append('jack_connect SuperCollider:out_1 system:playback_1')
        commands.append('jack_connect SuperCollider:out_2 system:playback_2')
    if record:
        commands.append('jack_connect SuperCollider:out_1 PulseAudio\ JACK\ Source:front-left')
        commands.append('jack_connect SuperCollider:out_2 PulseAudio\ JACK\ Source:front-right')


def kill_servers():
    '''
    Would be nice to figure out how to also close server window
    '''
    os.system('gnome-terminal -- sclang  {}/kill_servers.scd'.format(os.path.dirname(__file__)))


def boot_and_connect():
    boot_jack()
    boot_sc_server()
    connect_supercollider_to_jack()
"""